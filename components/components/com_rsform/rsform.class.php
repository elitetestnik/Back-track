...
class SManager {
	...
	function createRows( )
	{
		...
		//{
				...
				echo "<tr class=\"row$rowColor\">";
					...
					$querySubmissions = mysql_query("SELECT Username, DateSubmitted FROM `{$RSadapter->tbl_rsform_submissions}` WHERE `SubmissionId` = '{$row}'") or die(mysql_error());
	...
	if(!empty($this->exportOrder))
	{
		...
		//resolve duplicates
		foreach($this->exportOrder as $key=>$value){
			$this->exportOrder[$key] = $value.'.'.rand(0,100000);
		}
