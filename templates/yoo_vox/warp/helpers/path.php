...
/*
	Class: WarpHelperPath
		Path helper class, simplify path handling
*/
class WarpHelperPath extends WarpHelper {
    ...
    /*
		...
		Returns:
			Void
	*/
	function register($path, $namespace = 'default') {
	...
	*/
	function url($resource) {
	    ...
	    if ($url) {
	        ...
	        $url = $this->warp->system->url.'/'.ltrim(preg_replace('/'.preg_quote(str_replace(DIRECTORY_SEPARATOR, '/', $this->warp->system->path), '/').'/', '', $url, 1), '/');
	    }
	...
	/*
		...
		Returns:
			Array
	*/
	function files($resource, $recursive = false) {
	...
	/*
		...
		Returns:
			Array
	*/
	function dirs($resource, $recursive = false) {
	...
	/*
		...
		Returns:
			Array
	*/
	function ls($resource, $mode = 'file', $recursive = false) {
	...
	*/	
	...
	/*
		...
		Returns:
			Array
	*/	
	function _list($path, $prefix = '', $mode = 'file', $recursive = false) {
